        ...
        <div className="-my-6">
          {dataAboutPage.map((data) => (
            <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
              <h3 className="mb-1 text-lg sm:text-xl font-bold text-white sm:mb-0">{data.title}</h3>
              <div
              ...
              >
                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0 text-emerald-400 bg-emerald-950/50 border border-emerald-500/20 rounded-full">
                  {data.date}
                </time>
              ...
              <div className="text-sm text-slate-300 max-w-xl leading-relaxed mt-2">{data.description}</div>
            </div>
          ))}
        </div>
      </div>
